...
function generateDraft(client: string, industry: string, services: string[], relatedCases: Case[]) {
  const caseRef = relatedCases[0];
  const kpi = caseRef?.kpi ?? "成果を最大化";
  const caseTitle = caseRef?.title ?? "類似事例";

  return [
    {
      title: "① 現状認識と課題",
      content: `${client || "貴社"}は現在、${industry || "業界"}における競争環境の激化という課題を抱えておられると認識しております。特に${services.join("・")}の領域において、効果的な施策が求められている状況です。\n\n弊社では過去に「${caseTitle}」で${kpi}を達成した実績があり、同様の課題解決が可能と考えております。`,
    },
    {
      title: "② 提案概要",
    ...
    {
      title: "③ 実績・参考事例",
      content: relatedCases.slice(0, 2).map((c) =>
...
function DraftContent() {
  ...
  const preselectedId = params.get("case");
  ...
  const relatedCases = MOCK_CASES.filter((c) => {
    if (preselectedId) return c.id === preselectedId || c.industry === industry || c.services.some((s) => services.includes(s));
    return c.industry === industry || c.services.some((s) => services.includes(s));
  }).slice(0, 3);

  useEffect(() => {
    if (preselectedId) {
      const c = MOCK_CASES.find((c) => c.id === preselectedId);
      if (c) {
  ...
  const handleGenerate = () => {
    setLoading(true);
    setTimeout(() => {
      setDraft(generateDraft(client, industry, services, relatedCases));
      setGenerated(true);
      setLoading(false);
      setOpenIdx(0);
    }, 1200);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">条件を入力するとAIが過去事例を参照してドラフトを自動作成します</p>
      </div>
      ...
      <div className="bg-white border rounded-xl p-5 space-y-4">
            ...
            {INDUSTRIES.map((i) => (
              <button key={i} onClick={() => setIndustry(industry === i ? "" : i)}
                className={cn("text-xs px-3 py-1.5 rounded-full border transition-colors font-medium",
                  industry === i
                    ? `${INDUSTRY_COLOR[i]} border-current`
          ...
          <div className="flex flex-wrap gap-2">
            {SERVICES.map((s) => (
              <button key={s} onClick={() => toggleService(s)}
                className={cn("text-xs px-3 py-1.5 rounded-full border transition-colors font-medium",
                  services.includes(s)
                    ? `${SERVICE_COLOR[s]} border-current`
                    : "border-gray-200 text-gray-500 hover:border-gray-400"
                )}>
                {s}
              </button>
        ...
        {relatedCases.length > 0 && (
          <div className="bg-indigo-50 border border-indigo-100 rounded-lg px-4 py-3 space-y-1">
            <div className="flex items-center gap-2 text-xs font-semibold text-indigo-700">
              <Bot className="w-3.5 h-3.5" />参照予定の事例（{relatedCases.length}件）
            </div>
            {relatedCases.map((c) => (
              <p key={c.id} className="text-xs text-indigo-600 pl-5">・{c.title}</p>
            ))}
          </div>
        )}
        ...
        <button onClick={handleGenerate}
          ...
          {loading ? (
            <><span className="animate-spin">⟳</span>AI生成中...</>
          ) : (
